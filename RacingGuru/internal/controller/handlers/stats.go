...
func (h *Handler) StatsDriver(w http.ResponseWriter, r *http.Request) {
	...
	if driverID != "" {
		...
		if err != nil {
			h.sendLoggedError(w, r, "invalid driver_id", "INVALID_REQUEST", http.StatusBadRequest, err)
			return
		}
		driver.Id = parsedID
	} else {
...
func (h *Handler) StatsTeam(w http.ResponseWriter, r *http.Request) {
	...
	if teamID != "" {
		...
		if err != nil {
			h.sendLoggedError(w, r, "invalid team_id", "INVALID_REQUEST", http.StatusBadRequest, err)
			return
		}
		team.Id = parsedID
	} else {
